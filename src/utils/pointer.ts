...
export class Pointer extends THREE.Mesh {
  ...
  setFromTo(from: THREE.Vector3Like, to: THREE.Vector3Like) {
    this._origin.copy(from);
    this._target.copy(to);
    this.reposition();
  }

  setFromDir(from: THREE.Vector3Like, dir: THREE.Vector3Like, distance?: number) {
    this._origin.copy(from);
    this._target
      .copy(dir)
      .normalize()
      .multiplyScalar(distance || 1.0)
      .add(from);
    this.reposition();
  }
