...
export class XrHead {
  ...
  constructor(context: Context) {
    ...
    const head = new THREE.Mesh(new THREE.SphereGeometry(0.1), new THREE.MeshPhongMaterial({ color: "magenta" }));
    head.scale.set(1, 1.25, 1);
    this.headModel.add(head);
    const nose = new THREE.Mesh(new THREE.SphereGeometry(0.05), new THREE.MeshPhongMaterial({ color: "white" }));
